...
/* Klik notifikasi laporan baru: kalau dasbornya masih terbuka, tab itu
   difokuskan dan diminta membuka panel laporannya; kalau sudah ditutup,
   dasbor dibuka lagi dengan penanda laporan di alamatnya. */
self.addEventListener("notificationclick", function (e) {
  e.notification.close();
  const ts = (e.notification.data || {}).ts || "";
  e.waitUntil(
    self.clients.matchAll({ type: "window", includeUncontrolled: true }).then(function (daftar) {
      for (const c of daftar) {
        if ("focus" in c) { try { c.postMessage({ jenis: "rk-buka-laporan", ts: ts }); } catch (x) {} return c.focus(); }
      }
      if (self.clients.openWindow) return self.clients.openWindow("./" + (ts ? "#laporan=" + encodeURIComponent(ts) : ""));
    })
  );
});
